...
class BannerPic{
	...
	createUI(){
		...
		for(let i=0;i<this.imgs.length;i++){
			...
			asDom.href=this.as[i];
			// 创建a标签
			asDom.style.cssText=`width:100%;
			height:100%;
			display:block;
			position: absolute;
					left:${this.width}px;
			...
			imgDom.style.cssText = `position: absolute;
					...
					height: 100%;`;
			if(i==0){
				// imgDom.style.left = "-328px";
				imgDom.style.left=this.imgLeft+"px";
				asDom.style.left="0px";
			}

			asDom.appendChild(imgDom);
			this.boxDom.appendChild(asDom);
			this.imgDoms.push(asDom);
		}
		...
		switch(this.doudouDirection){
			case "上":
						...
						break;
			case "下":
						ulDom.style.bottom=this.doudouButton+"px";
						ulDom.style.left= (this.width-this.douSize*2*this.imgs.length) +"px";
						break;
		}
